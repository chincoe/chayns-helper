...
export const HttpStatusCode: {
     ...
     * inefficient. To have a server check the request's headers, a client must send Expect: 100-continue as a header
     * in its initial request and receive a 100 Continue status code in response before sending the body. The response
     ...
     * and the response is a representation of the result of one or more instance-manipulations applied to the
     * current instance.
     */
    ImUsed: 226,
     ...
     * negotiation). For example, this code could be used to present multiple video format options, to list files
     ...
     * If-Modified-Since or If-None-Match. In such case, there is no need to retransmit the resource since the
     ...
     * changed when reissuing the original request. For example, a POST request should be repeated using another
     * POST request.
     */
    TemporaryRedirect: 307,
     ...
     * applicable to the requested resource. See Basic access authentication and Digest access authentication. 401
     * semantically means
     ...
     * digital cash or micro payment scheme, but that has not happened, and this code is not usually used. Google
     * Developers API uses this status if a particular developer has exceeded the daily limit on requests.
     */
    PaymentRequired: 402,
     ...
     * Most use cases do not require clients and search engines to purge the resource, and a "404 Not Found" may be
     * used instead.
     */
    Gone: 410,
     ...
     * a query-string of a GET request, in which case it should be converted to a POST request. Called "Request-URI
     * Too Long" previously.
     */
    UriTooLong: 414,
     ...
     * Coffee Pot Control Protocol, and is not expected to be implemented by actual HTTP servers. The RFC specifies
     * this code should be returned by teapots requested to brew coffee. This HTTP status is used as an Easter egg
...
export type HttpStatusCodeType = typeof HttpStatusCode[keyof typeof HttpStatusCode] | number;
