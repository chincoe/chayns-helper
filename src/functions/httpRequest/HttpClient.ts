...
export default class HttpClient<TDConfig extends Partial<HttpRequestConfig>,
    ...
    ): Promise<CombinedRequestResult<TOptions, TDOptions>> {
...
// RESPONSE HANDLING
    ...
    public typedFetch<TJson extends Record<string, any>>(jsonTemplate: TJson): <TOptions extends HttpRequestOptions>(
        ...
        processName?: string,
        options?: TOptions
    ) => Promise<CombinedRequestResult<TOptions, TDOptions, TJson>>;
    ...
    ): Promise<CombinedRequestResult<TOptions, TDOptions, DefaultType<TJson, Record<string, any>>>> {
        return this.fetch(
            address,
            config,
            processName,
            options
        ) as Promise<CombinedRequestResult<TOptions, TDOptions, DefaultType<TJson, Record<string, any>>>>;
    }
}
